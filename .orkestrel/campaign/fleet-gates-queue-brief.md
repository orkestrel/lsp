# Unit brief: fleet-gates-queue — the release verifier sweep
...
Run the authoritative tree-wide gate chain in `/home/user/queue` and report exit-code
truth, as the independent reading the campaign's exit criterion requires for every
touched checkout.

## Context

- Subject tree: `/home/user/queue` on its checked-out branch. Confirm
...
Run, in order, from `/home/user/queue`:
