# Unit brief: fleet-gates-process — the release verifier sweep
...
Run the authoritative tree-wide gate chain in `/home/user/process` and report exit-code
truth, as the independent reading the campaign's exit criterion requires for every
touched checkout.

## Context

- Subject tree: `/home/user/process` on its checked-out branch. Confirm
...
Run, in order, from `/home/user/process`:
