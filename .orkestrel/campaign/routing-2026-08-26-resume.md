...
## Routing rows added at the m4-proof and h2.3-fences boundary, 2026-08-26

| Unit | Role | Engine | Standing |
|---|---|---|---|
| `h2.3-fences` | `implementer` | Opus 5 (native) | Landed at markdown `e5876ff`; Orchestrator re-ran the gates after the writer exited: oxfmt check exit 0, `src:core` `599 passed (599)` exit 0. |
| `m4-proof` | `implementer` | Opus 5 (native) | Landed at mcp `0fe1879`; Orchestrator re-ran the gates after the writer exited: oxfmt check exit 0, four-file scoped suite `596 passed (596)` exit 0, `git diff -- src/` empty. |
