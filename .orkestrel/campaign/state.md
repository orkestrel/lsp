...
| M — mcp | Era 2026-07-28 native, repairs, subscriptions, the tasks family | **M4 ACCEPTED at `11c879c`** (`mcp/m4-acceptance.md`). **M5 CLOSED with no code change** (`mcp/m5-closure.md`): the producer, registration, and receipt halves retained on the Grok sweep's evidence, and the marker half intentionally excluded by the user's no-deprecation ruling — a fleet sweep reads zero `@deprecated` hits across every repository's source. M6 (naming cascade, ruled — see the routing ledger) runs from `11c879c`, last so mcp bumps once. |
