...
| M — mcp | Era 2026-07-28 native, repairs, subscriptions | M3 ACCEPTED 2026-08-26 through `b50520a` under the reconciled verdict (`mcp/m3-round-audit-verdict.md`): the M3.2 repairs closed the analyst's accessor-signal and progress-routing findings with mutation probes and a green host gate chain; next M4 (tasks proof), M5 (deprecated surface), M6 naming cascade (user blessing pending) |
...
The campaign record lives in the lsp repository under `.orkestrel/`, one folder per
package — `lsp/`, `mcp/`, `markdown/`, `html/`, `probe/`, `workflow/` — with the
cross-package record (the plan, the original audit, this state file, the opening
design round, the fleet sweeps, the shared research, and the H-wave terrain) under
