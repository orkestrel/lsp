...
|---|---|---|---|
| `orkestrel/lsp` | `main` | `759b899` | The campaign's subject and its record; the archive lives here under `.orkestrel/`. L2 through L6 accepted, the guides parity project and the vocabulary pass landed, and the fleet verifier read the full chain GREEN at `759b899` on 2026-08-26. The exit criterion is met except the prune; the prune and the release run on the user's word, and `ROADMAP.md` carries the campaign's close and the forward findings. |
...
| `orkestrel/mcp` | `claude/lsp-spec-audit-est33d` | `aa20c37` | **M4 ACCEPTED** (`mcp/m4-acceptance.md`). The round audit's reconciled findings closed in `m4.1-repairs` (Opus, `11c879c`); the fix round closed on the checker's PASS over every claim (`m4.1-checker-verdict.md`) beside the Orchestrator's gate re-run (conformance `42`, guides `144`, scoped core `596`, every exit 0). M5 closed with no code change (`mcp/m5-closure.md`) under the user's no-deprecation ruling. **M6 ACCEPTED at `aa20c37`** (`mcp/m6-acceptance.md`): the ruled cascade behind the checker's PASS on every claim (`mcp/m6-checker-verdict.md`), the Orchestrator's own green gates, and the fleet verifier's GREEN. The M wave is closed and the mcp bump rides the release. |
...
| `queue`, `tool`, `process`, `middleware` | `claude/lsp-spec-audit-est33d` | G1 commits; process `2a47ed1` | **G1 ACCEPTED.** Each guide records why its package stays untouched by the progress work, landed by parallel `builder` units, ruled PASS on every claim by the `checker` (`campaign/g1-check-verdict.md`), committed and pushed. The process checkout also carries `2a47ed1`: the g2-orphan-fixture guard closing the fleet verifier's one red row (the unguarded `late` fixture holder crashed on its destroyed held pipe's stream error; red-first, mutation-bound, and the Orchestrator's deciding re-run read `56 passed | 1 skipped` exit 0). The clean re-verify runs after the commit. |
...
| M — mcp | Era 2026-07-28 native, repairs, subscriptions, the tasks family | **M4 ACCEPTED at `11c879c`** (`mcp/m4-acceptance.md`). **M5 CLOSED with no code change** (`mcp/m5-closure.md`): the producer, registration, and receipt halves retained on the Grok sweep's evidence, and the marker half intentionally excluded by the user's no-deprecation ruling — a fleet sweep reads zero `@deprecated` hits across every repository's source. **CLOSED.** M4 accepted, M5 closed under the no-deprecation ruling, M6 accepted at `aa20c37` behind the checker's PASS and the fleet verifier's GREEN. |
...
no fence); the html and markdown reused-identity engine divergence; `LSPServer`; transport-ingress backpressure (mcp); the html-spans-to-markdown inbound
...
list in `plan.md`; the session-assembly duplication (a Cloud session that attaches every
fleet repository injects each checkout's `CLAUDE.md`, `AGENTS.md`, `orchestration.md`,
and auto-loaded rules into the model context per window even though the copies are
byte-identical scaffold propagations — a scaffold-owned slimming decision, or attaching
only the repositories a session works, closes it; deleting the vendored files from
targets is not a fix, because `repair` restores them and the policy gates read them).
