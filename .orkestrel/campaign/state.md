...
| `queue`, `tool`, `process`, `middleware` | `claude/lsp-spec-audit-est33d` | lockfile commits | Clean and pushed. Not campaign subjects for code; each owes one G1 guide ruling recording why it stays untouched by the progress work, per `campaign/g1-terrain-distillate.md`, with `builder` units dispatched across the four disjoint checkouts. |
...
| `h2.2-prose` | `implementer` — Opus native | **Returned; audit queued.** Every enumerated site corrected or re-ruled against executed probe readings, the type openers verbed, and the reviewer's claim-5 ruling on `guides/markdown.md:316-319` overturned on evidence — its separate-inputs clause was false and is corrected. The unit's shared-file patch (`src/core/Markdown.ts` `@returns`) is integrated serially. Gates green scoped, `592 passed (592)` unchanged, guides parity `18 passed`. Records: `markdown/h2.2-prose-brief.md`, `-report.md`. The Sol `analyst` audit runs next on the held tree. |
| `h2.3-fences` | `implementer` — Opus native | Queued, after the `h2.2-prose` audit. R4, plus one carried finding from the `h2.2-prose` report: the `src/core/Markdown.ts` class TSDoc at `:27-28` still carries the parse-time over-claim and needs the rewrite the `MarkdownInterface.span` remarks received. Auditor: Sol `analyst`. |
