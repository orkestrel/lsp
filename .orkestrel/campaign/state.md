...
| `orkestrel/mcp` | `claude/lsp-spec-audit-est33d` | `aa20c37` | **M4 ACCEPTED** (`mcp/m4-acceptance.md`). The round audit's reconciled findings closed in `m4.1-repairs` (Opus, `11c879c`); the fix round closed on the checker's PASS over every claim (`m4.1-checker-verdict.md`) beside the Orchestrator's gate re-run (conformance `42`, guides `144`, scoped core `596`, every exit 0). M5 closed with no code change (`mcp/m5-closure.md`) under the user's no-deprecation ruling. **M6 LANDED at `aa20c37`**: the ruled cascade with zero old-spelling residue, the header and the `MCPLegacy` family untouched, and the Orchestrator's one-sentence fallback-TSDoc repair (the routing ledger records the ruling); the Orchestrator's own gates read green (check, guides `144`, conformance `42`, each exit 0). The checker closes the round, then acceptance and the mcp fleet verifier. |
...
| M — mcp | Era 2026-07-28 native, repairs, subscriptions, the tasks family | **M4 ACCEPTED at `11c879c`** (`mcp/m4-acceptance.md`). **M5 CLOSED with no code change** (`mcp/m5-closure.md`): the producer, registration, and receipt halves retained on the Grok sweep's evidence, and the marker half intentionally excluded by the user's no-deprecation ruling — a fleet sweep reads zero `@deprecated` hits across every repository's source. **M6 LANDED at `aa20c37`** (the ruled cascade; the routing ledger records the landing and the fallback-sentence ruling); the checker closes the round, then acceptance and the mcp fleet verifier. |
